...
(
  pv_item_title IN VARCHAR2
) IS

  lv_item_title VARCHAR2(30) :=pv_item_title;

  CURSOR get_files IS
    SELECT i.item_id, fl.file_name
    FROM item i
    JOIN file_list fl
...
BEGIN
  ...
  dbms_output.put_line(lv_item_title);

  FOR answer IN get_files
    LOOP
--      dbms_output.put_line('Answer: ITEM ID '||answer.item_id||' ::: FILE NAME: '||answer.file_name);

      load_clob_from_file( src_file_name     => answer.file_name
                       , table_name        => 'ITEM'
                       , column_name       => 'ITEM_DESC'
                       , primary_key_name  => 'ITEM_ID'
                       , primary_key_value => TO_CHAR(answer.item_id) );
    END LOOP;
